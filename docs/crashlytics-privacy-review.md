...
별도 이메일 요청 없이 파기 절차를 시작하는 정책을 초안에 반영했다. 개별 삭제·처리정지 요구는
...
근거를 검토하도록 설명한다. 이번 초안은 탈퇴 후의 별도 진단 목적 보유를 추가하지 않고,
...
이 문서는 확정한 파기 정책을 반영한 초안이며, 구체적인 삭제 수단은 후속 작업에서 확정한다.
삭제 기능의 구현·검증은 아직 완료되지 않았다. 게시 전에는 [이슈 #22](https://github.com/soma17th-369/Laimory-landing-page/issues/22)에서
...
약관은 구현할 정책을 적은 초안이며 위 확인 전까지 PR을 Draft로 유지한다. Google 측 삭제 API나
설치 식별자 삭제의 효과, 수탁자 시스템 전체의 삭제 완료 시한을 확인한 것으로 표현하지 않는다.
