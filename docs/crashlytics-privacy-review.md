...
개인정보 처리방침 `1.0`이다. 공개 원문은 [개인정보 처리방침](../public/terms/privacy-policy/1.0.html),
구현·게시 추적은 [#22](https://github.com/soma17th-369/Laimory-landing-page/issues/22)에서 관리한다.
...
계정 정보의 탈퇴 후 5일 파기와 수탁자 시스템의 삭제 절차를 구분하되, 오류 진단 정보와 관련
식별자도 탈퇴 시 파기 대상에 포함한다. 기본 90일이 지나기 전이라도 탈퇴 등 파기 사유가 발생하면
별도 이메일 요청 없이 파기 절차를 시작하는 정책을 반영했다. 개별 삭제·처리정지 요구는
처리방침 제12조의 접수 경로를 유지한다. 실제 삭제 수단의 확인 상태는 제6절에서 별도로 다룬다.
...
위 통합 안내서 52~53쪽은 계약 종료 후에는 계약 이행 근거를 계속 적용하지 않고 다른 적법한
근거를 검토하도록 설명한다. 이번 처리방침은 탈퇴 후의 별도 진단 목적 보유를 추가하지 않고,
탈퇴 시 파기 절차를 시작하는 정책을 채택한다. 국내 처리의 근거만 정당한 이익으로 바꾸어도
제28조의8의 국외 보관 요건이 자동으로 충족되는 것은 아니다.
...
개인정보 처리방침 제1·3·4·6·7·12조를 정리하고, 제9조의 자동 마스킹 문구는 실제 적용 대상인
인공지능 실행 추적정보로 한정한다. 앱 자동 오류 메시지에도 같은 마스킹이 적용된다고 읽히지 않게 한다.
문서 `1.0`·기존 시행일·URL을 유지하며 #5를 비롯한 다른 동의서와 Android 구현은 변경하지 않는다.
약관 본문과 스타일은 이 저장소의 HTML에서 직접 관리한다. 서버의 과거 Markdown·생성기와 동기화하지 않는다.
HTML의 내용·표·metadata 및 정적 사이트 빌드 검증은 운영 게시 검증과 구분한다.

## 6. 확정한 파기 정책과 삭제 수단 후속 작업
...
- 회원 탈퇴 시 오류 진단 정보와 관련 식별자도 파기 대상에 포함하고, 별도 이메일 요청 없이
  지체 없이 파기 절차를 시작한다. 수탁자 삭제 요청 등 필요한 조치를 하고 처리 결과를 확인한다.
- 회원 탈퇴와 별개인 삭제·처리정지 요구 및 문의는 `laimory369@gmail.com`으로 접수한다.
- 제1·3·7조의 보유기간 표에 탈퇴 등 파기 사유가 기본 90일보다 먼저 적용될 수 있음을 명시한다.
- 제12조는 법령에 따른 필요한 조치와 결과 안내를 명시한다. Crashlytics의 권리 행사를
  일괄 제외하거나 Google의 기본 보유기간이 지나기까지 기다리도록 하는 단서를 두지 않는다.
...
확정한 파기 정책을 담은 처리방침을 먼저 게시하고, 구체적인 삭제 수단의 구현·검증은 서비스
운영팀의 후속 작업으로 진행한다. 삭제 기능의 구현·검증은 이 PR에서 확인되지 않았다.
[이슈 #22](https://github.com/soma17th-369/Laimory-landing-page/issues/22)를 열린 상태로 유지하여
다음 사항을 확인하고 제4조·제7조·제12조에 기재한 정책과 실제 동작을 일치시킨다.

- 탈퇴 또는 이메일 요청을 처리할 담당자와 처리 대상 식별 방법
- Google에 삭제를 요청하는 실제 절차 및 그 절차가 삭제하는 데이터의 범위
- 별도 이메일 요청 없이 탈퇴에 따라 삭제 절차가 시작되고 실제로 처리되는지 여부
- 삭제 완료 확인 방법과 소요 기간. 절차 시작이나 요청 접수만으로 파기가 완료됐다고 표시하지 않음

자동화 자체가 법에서 지정한 유일한 방법은 아니지만 실제 수행 가능한 절차가 필요하다.
약관 게시를 삭제 수단의 구현·검증 완료로 취급하지 않는다. Google 측 삭제 API나 설치 식별자
삭제의 효과, 수탁자 시스템 전체의 삭제 완료 시한을 확인한 것으로 표현하지 않는다.
