...
반대 방향으로는 `vercel.json`의 `redirects`가 `.html`이 붙은 주소를 확장자 없는
정식 주소로 308(영구 이동) 리다이렉트해 검색엔진에 중복 주소가 남지 않게 합니다.
**새 버전을 게시할 때는 rewrite와 redirect를 한 줄씩 함께 추가하세요.**
(자세한 배경은 `docs/seo.md`의 리다이렉트 정책 참고)
...
개인정보 처리방침·이용약관·위치약관 및 두 동의서 HTML에 Google 지도·주소 변환을 반영합니다.
사용자는 Google에 문의하지 않고 이전 국가를 `미국 및 Google이 관련 서비스 서버를 운영하는 국가`,
보유기준을 `Google의 개인정보처리방침 및 보관 정책에 따라 서비스 운영·법령상 필요한 기간`으로
기재하도록 지시했습니다. 관련 Google 정책을 연결하고 지도 표시·주소 변환의 항목과 시점을 구분합니다.

[Google 지도·주소 변환 검토 문서](google-location-terms-review.md)에 반영 범위와 근거의 한계를 둡니다.
이 문서는 게시본 원문이 아니며 `public/` 또는 빌드 결과로 복사하지 않습니다.
포괄적 문구의 반영을 전체 이전 국가·개별 보유기준이나 법적 적정성의 검증 완료로 취급하지 않습니다.
동의·권리 절차의 추가 검증은 사용자 요청에 따라 이번 작업에서 제외하고 운영팀의 적용 범위로 둡니다.
...
4. `vercel.json`의 `rewrites`와 `redirects`에 `1.1` 항목을 한 줄씩 추가합니다.
   **`1.0` 항목은 남겨 둡니다.** 기존 버전에 동의한 사용자가 자신이 동의한 문서를
   계속 열 수 있어야 합니다.
5. 푸터가 가리키는 버전을 올리려면 `src/lib/links.ts`의 `TERMS_URL`을 수정합니다.
   사이트맵의 약관 주소도 이 값을 읽으므로 함께 갱신됩니다.
