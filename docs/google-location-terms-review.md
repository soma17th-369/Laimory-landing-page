...
| 이용약관 | 제9조에 Google Maps 추가약관·개인정보처리방침 링크 | 지도·주소 변환 동의 절차와 함께 검토 |
| 위치기반서비스 이용약관 | 제4·8·11조에서 홈·초안 확인 중의 처리와 최종 AI 전송 선택을 구분 | 아래 Google 제공 및 통보 검토 결과 반영 |
| 제3자 제공 동의 | 아래 수정안을 작성. 현재 `public` HTML에는 미반영 | Google 보유기준과 동의·철회 적용 범위를 확정하여 반영 |
| 국외 이전 동의 | 아래 수정안을 작성. 현재 `public` HTML에는 미반영 | 국가·보유기준을 확정하여 반영 |

**세 HTML의 사실 안내만으로 Google 관련 고지·동의가 모두 완성되는 것은 아니다.** 이 PR 전체는 아래 확인사항이 해결될 때까지 Draft로 유지한다. 미확정 필드를 공개 HTML에 넣지 않고, 이 검토 문서는 Astro의 게시 경로 밖에 둔다. 자동 배포 설정은 변경하지 않는다.
...
**적용 조건:** Google 항목의 거부가 기존 AWS 동의와 어떻게 구분되는지 동의 화면과 대조한다. 기존 문서 하나에 공급자를 추가하는 것만으로 공급자별 동의·철회 기능이 구현되지는 않는다. AWS 전체 동의 거부 안내와 Google만 거부할 때의 효과를 혼동하지 않도록 최종 문구를 확정한다. 위 문구는 해당 처리 중단 절차가 확인된 이후 게시한다. 앱 삭제나 최종 위치 선택 해제를 Google에 대한 별도 동의 철회 수단이라고 쓰지 않는다.
...
Maps 문서는 위 7개국을 데이터센터 범위로 안내한다. 원격 접근·지원 등까지 포함한 전체 이전 국가를 확인한 목록으로 과장하지 않는다. Geocoder가 Maps의 계약·처리 범위를 그대로 따른다는 근거도 아직 확인되지 않았다. Google 위치 서비스의 확인자료 6개월, 계정 활동 18개월, 개발자 캐시 30일 등 다른 정보의 보유기간을 여기에 대입하지 않는다.
...
2. **동의·권리 절차:** Google 제공·국외이전 동의의 범위, 첫 요청 전 동의, 거부·철회 후 중단과 위치정보 제공 통보의 적용 여부를 확인한다. Android는 이 작업에서 수정하지 않는다.
3. 위 수정안을 두 동의서 HTML과 처리방침·위치약관에 반영하고 이 Draft 상태 표시를 실제 결과에 맞춰 갱신한다. 최초 게시 후 새 세션에서 본문·URL을 확인한다.
